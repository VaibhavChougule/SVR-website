...
const Dashboard = () => {
    ...
    const handleCategoryClick = async (category) => {
        ...
        try {
            ...
            const statusOrder = { OPEN: 1, pending: 2, feedbackWaiting: 3, Completed: 4 };
    ...
    const handleAssignEngineer = async (ticketId) => {
        ...
        try {
            setLoading(true);
            await fetch(`${config.API_URI}/api/engineerAssignment`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ ticketId, Engineer: engineerName }),
            });
    ...
    const handleClose = async (ticketId) => {
        try {
            setLoading(true);
            await fetch(`${config.API_URI}/api/ticketClose`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    const allKeys = Array.from(new Set(popupData.flatMap((item) =>
    ...
    const renderContent = () => {
        ...
        //         case "feedbackWaiting": return "bg-yellow-500 text-black";
        ...
        const getStatusStyle = (status) => {
            switch (status) {
                case "OPEN": return "text-red-500 font-bold";
                case "pending": return "text-orange-500 font-bold";
                case "feedbackWaiting": return "text-yellow-500 font-bold";
            ...
            case "Maintenance":
                return (
                    <div className="bg-slate-700 p-6 rounded shadow-md overflow-x-auto">
                        <h2 className="text-2xl font-semibold text-sky-400 mb-4">Maintenance</h2>
                        <input
                            type="text"
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            placeholder="Search by Ticket ID"
                            className="mb-4 p-2 rounded border border-slate-600 bg-slate-600 text-white w-full"
                        />
                                ...
                                {filteredData.map((item, index) => (
                                    <tr key={index} className="bg-slate-700 hover:bg-slate-600 text-sm">
                                        {allKeys.map((field) => (
                                            <td
                                                key={field}
                                                className={`px-4 py-2 border border-slate-600 ${field === "status" ? getStatusStyle(item[field]) : ""
                                                    }`}
                                            >
                                            ...
                                            <>
                                                <td className="px-4 py-2 border border-slate-600">
                                                    ...
                                                    >
                                                        ...
                                                        {["Ankita Bhosale", "Sanket Patil", "Jayesh Jadhav", "Vaibhav Chougule"].map((name) => (
                                ...
                                {popupData.map((item, index) => (
                                    <tr key={index} className="bg-slate-700 hover:bg-slate-600 text-sm">
                                        ...
                                        {item.status !== "Completed" ? (
                                            ...
                                            <td className="px-4 py-2 border border-slate-600 text-center text-gray-400">
                                                Completed
                                            </td>
                                        )}
                                    </tr>
        ...
        return (
            <div className="bg-slate-800 text-white min-h-screen flex justify-center items-center p-4">
                <form onSubmit={handleLogin} className="bg-slate-700 p-6 rounded shadow-md w-full max-w-md">
                    ...
                    <div className="mb-4">
                        <label className="block text-sm mb-2">Admin ID</label>
                        <input
                            type="text"
                            value={adminId}
        ...
        <div className="bg-slate-800 text-white min-h-screen p-6">
            ...
            {loading && (
                <div className="fixed inset-0 bg-black bg-opacity-80 flex justify-center items-center">
                    <div className="bg-slate-700 p-4 rounded">Loading...</div>
                </div>
            )}
